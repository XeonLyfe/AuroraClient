...
public class MobESPMod extends Mod {
	public transient ModAttributeBoolean tracers = new ModAttributeBoolean("Tracers", false);
	public transient ModAttributeBoolean showAnimals = new ModAttributeBoolean("ShowAnimals", true);
	...
	public MobESPMod() {
		super("mobesp", "Mob ESP", "Highlights nearby mobs.", ModCategoryEnum.RENDER);

		addAttrib(tracers);
		addAttrib(showAnimals);
	...
	public void onLocalPlayerUpdate() {
		...
		for (Entity entity : Minecraft.getMinecraft().world.loadedEntityList) {
			if (!(entity instanceof EntityLiving))
				continue;
			if (entity.isDead || ((EntityLiving) entity).getHealth() < 0)
				continue;
			if (EntityUtil.isAnimal(entity) && !showAnimals.value)
				continue;
